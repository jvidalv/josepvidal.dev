    ...
    <>
      ...
      <section className="font-serif">
        <p className="text-neutral-600 dark:text-neutral-400 text-xl">
          Considering Spain life expectancy of 83 years, and that I was born in 1994, this is a
          weekly divided visual representation of my sentient time left.
        </p>
      </section>
      <section className="font-serif">
        <div className="grid gap-1 mt-12">
          {Array.from({ length: lifeExpectancyInYears }).map((_, x) => {
              ...
              <>
                <div key={x} className="relative flex ">
                  ...
                  {shouldGuideBeDisplayed && (
                    <div className="absolute left-full text-xs ml-1.5 -mt-1">{x + 1}</div>
                  )}
                </div>
        ...
        <cite className="block text-sm mt-8">
          ...
          that it has passed away before we knew it was passing. So it is: we are not given a short
          life but we make it short, and we are not ill-supplied but wasteful of it… Life is long if
          you know how to use it.” ~ Seneca
        </cite>
      </section>
    </>
  );
